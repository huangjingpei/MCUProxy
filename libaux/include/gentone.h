...
typedef struct dtmf_tone_inst_t_
{
    ...
    short reinit; /* non-zero if the oscillator model should
     be reinitialized for next event */
    short oldOutputLow[2]; /* oscillator recursion history (low tone) */
    short oldOutputHigh[2]; /* oscillator recursion history (high tone) */
...
extern "C" {
#endif
extern short GenDtmf(dtmf_tone_inst_t *DTMFdecInst,
                char  value,
                short volume,
                short *signal,
                short sampFreq,
...
extern short GenTone(dtmf_tone_inst_t *DTMFdecInst,
                short *yInit,
                short *aTbl,
                short volume,
                short *signal,
...
extern int  get_tone_ceff(int q, int fs, int f,short *yInit,short *aTbl);
#ifdef __cplusplus
}
#endif
